...
Service_SSR(){
if [[ ${OS} = "centos" ]]; then
wget --no-check-certificate https://raw.githubusercontent.com/hybtoy/ssrrmu/master/ssr_centos -O /etc/init.d/ssr
chmod +x /etc/init.d/ssr
chkconfig --add ssr
chkconfig ssr on
else
wget --no-check-certificate https://raw.githubusercontent.com/hybtoy/ssrrmu/master/ssr_debian -O /etc/init.d/ssr
chmod +x /etc/init.d/ssr
...
Start_SSR(){
	check_pid
	wget -O /etc/init.d/ssr "{GitUser}/install/ssr.sh"
...
Install_SSR
cd
wget -O /usr/bin/ssr ${GitUser}/install/ssr.sh && chmod +x /usr/bin/ssr
touch /usr/local/shadowsocksr/akun.conf
rm -f /root/ssr.sh
echo "0 0 * * * root xpssr" >> /etc/crontab
